...
| `pack_install.test.ts` | what the pack migration added: `country_packs` with the checksum of the files on disk, the version each company copied, the fallback a pre-pack installation gets, a chart installed in Dutch and one installed in English, a corrected pack that changes the template and leaves the company alone, and row level security on the two new tables |
